...
parser.add_argument('--data_path', type=str, default="./Iris_data.txt")
...
validata_loader = DataLoader(validate_dataset, batch_size=1, shuffle=False)
test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False)
print("训练集大小：", len(train_loader) * opt.batch_size, "验证集大小：", len(validata_loader), "测试集大小：", len(test_loader))
...
def infer(model, dataset, device):
    ...
    accuracy = acc_num / len(dataset) #计算准确率
    return accuracy
...
def main(args):
    ...
    loss_function = nn.CrossEntropyLoss() # 定义损失函数 交叉Entropy
    ...
    for epoch in range(opt.epochs):
        ...
        ############################################################## validate 验证集 ######################################################
        val_accurate = infer(model=model, dataset=validata_loader, device=device)
